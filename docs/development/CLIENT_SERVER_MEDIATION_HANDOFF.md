...
- **⭐ ACTIVE (server agent — `cloud.kimball.home`):** `feature/android-unifiedpush` — **UnifiedPush push transport (privacy-first)**: ntfy on **loopback**, exposed as a **Host-routed `push.<domain>` on the app's own 443** (no new port, no new firewall rule; needs a DNS record + certificate entry for that name — the path route is impossible, measured 2026-09-18) + the server-side transport/registration/payload work. **AUTHORISED — see the Active Handoff below.** Canonical spec `docs/ANDROID_UNIFIEDPUSH_PLAN.md`. The Android half (plan Phases 1, 2 + client-side Phase 5) is **DONE and on-device verified** (2026-09-18) and does **not** block this work.
...
## ⭐ Active Handoff — Server: UnifiedPush push transport (privacy-first) → `cloud.kimball.home` (2026-09-18; topology settled — Host-based `push.<domain>`)

> ⭐ **AUTHORISED BY THE OPERATOR (2026-09-18) — this is the Active Handoff; start here.** The design is complete, every open question is answered, and the topology is settled, so there is nothing to wait for.
...
> **All operator answers are in hand (2026-09-18):** every §9 item is resolved (plan §9); **§7.1 step 1 is ANSWERED — a push server URL containing a path is rejected**; **§9.10 is APPROVED — use the Host-routed `push.dotnetcloud.net` on the same 443** (a DNS record + certificate entry for that name; no new port, no new firewall rule). Requirement §1.5's "no extra hostname" was accepted as unachievable.
> **The Android half is DONE** (client agent — `monolith`, on-device verified), so the §7 live E2E is the join point: when you finish, hand it back for the end-to-end run.
...
**Status:** ⭐ **ACTIVE — authorised to start (operator, 2026-09-18).** Server code untouched so far; the design, spec and every operator decision are complete. ⚠️ The Android half is **done** (2026-09-18) and its device test **changed the topology in Part 1** (a path URL is rejected).
...
1. **✅ First task — ANSWERED 2026-09-18 by the client agent; do NOT re-test it.** The distributor **rejects a push server URL containing a path**, so `<server-url>/<topic>` with a path cannot be used. Evidence (ntfy Android app against a self-hosted ntfy — the client agent's local rig, `http://192.168.0.154:2586`, was the control):
   ...
   - A separately-ported ntfy (`listen-https` + key/cert + a new NAT rule) is the **last-resort fallback** only.
