...
**Branch:** the chat-alerts feature branch (server + client halves both on it).
**Canonical spec:** `docs/ANDROID_CHAT_BACKGROUND_ALERTS_PLAN.md` (design, adopted), including its **"As built"** section (file list + findings). Read that first; it supersedes the older drafts throughout the document.

### ✅ Server half — DONE, deployed to `cloud.kimball.home` (2026-09-19, server agent — `cloud`)

The aggregate itself is live and needs no further server work. The deploy shipped `GET /api/v1/chat/alerts` (`ChatController.GetAlertsAsync` → `IChannelMemberService.GetAlertsAsync`): a constant-cost aggregate over `{ v, unread, mentions, unmutedUnread, unmutedMentions, topChannelId, changedAt }`, with a deterministic SHA-256 `ETag` and `If-None-Match` → `304`. Purely additive — `GET /api/v1/chat/unread` and `GetUnreadCountsAsync` are untouched, and there are **no migrations**. No separate push infrastructure was needed or built. **The only server-side item left is the proxy fix below.**
...
### ✅ Client E2E — DONE, on-device verified (2026-09-19, client agent — `monolith`, phone R5CWC356B2K)

Run with the branch's own arm64 Debug build (`adb install -r --no-incremental`; build **0 warnings / 0 errors**, Android tests **433 pass / 1 skip**), signed in to `cloud.dotnetcloud.net`:

| Check | Result |
| --- | --- |
| Job **3108** (`ChatAlertJobService`) | `PERSISTED`; `Minimum latency +5m0s`, `Max execution delay +6m0s`; **any** network (`NOT_BANDWIDTH_CONSTRAINED`) — **not** the media job's unmetered `3107` |
| Poll through the gateway, authenticated | `GET /api/v1/chat/alerts` → **`200`** with the app's own bearer token (the pre-deploy `404` is gone) |
| Headless wake with the app **not running** | the job starts the process (`Background chat alert poll started (headless)`), polls and re-arms itself (`scheduled in 60s` while unread, `300s` when idle) |
| **The alert itself** | message sent while the app process was **dead** → the poll logged `finished (Alerted, pending: True)`, `posted=True`, `foreground=False`, and posted **one generic notification**: `channel=chat_messages`, `android.title=String (New message)`, `android.text=String ()` — **no body, no sender, no channel name** |
| Tap → channel | the tap opened **exactly** the alert's channel (`groupKey=dnc_chat_019fd4f0-…` → `MessageListViewModel … STARTED for channel 019fd4f0-…`) |
| Read clears the alert | after reading, the next poll logged `Suppressed, pending: False` and fell back to the idle `300s` cadence, posting **no** second notification |
| Muted channel | message into a **muted** DM → `Suppressed, pending: False`, **no notification at all** (mute is absolute — the server's `unmutedUnread` stays 0 while `unread` counts it) |
...
| No foreground service | `dumpsys activity services net.dotnetcloud.client` → **empty** (log shows `ChatConnectionService destroyed`); no `startForegroundCount` |

### ✅ FIXED + VERIFIED (server agent — `cloud`, 2026-09-19): the core proxy no longer forwards headers twice, so `304` now happens

**Found while running the E2E above; fixed and unit-tested on `monolith` (see the commit), then deployed and re-verified on `cloud`.**

#### Deploy + `304` re-verify results (server agent — `cloud`, 2026-09-19)

| Check | Result |
| --- | --- |
| `dotnet build` (solution filter + CLI) | **0 warnings / 0 errors** |
| `ModuleApiProxyTransformerTests` | **4/4 pass** (written failing-first against the old code) |
| `sudo ./scripts/deploy.sh --force --verify` | **15/15 targets**, all core + module-host hashes verified |
...
| `/health/ready` | **Healthy — 14 module(s), all healthy**; `database` Healthy |
| `_framework/blazor.web.js` | **200** |
...
Leg 1 → leg 2 is the direct evidence: with the old duplication the module host saw `"…","…"`, could never equal the ETag, and always answered `200` with the full aggregate body.

Reproduce with the new reusable script (it authenticates through `/auth/session/login` + PKCE, so the account must have **no MFA**; note the login binds the credential as `[FromForm] username`, **not** an email field):

```bash
DNC_EMAIL=<username> DNC_TARGET=/api/v1/chat/alerts ./scripts/verify-module-proxy-if-none-match.sh
# → leg 1 200 + ETag, leg 2 304 empty body, "PASS: conditional GET honoured"
```


- **Symptom:** the client sends `If-None-Match: "EA7D5405675524624C3DFFCA9617CCE8"` and the server answers **`200`** carrying **the same** `ETag` — the token matches, yet the conditional still misses, so every poll re-computes and re-sends the aggregate.
- **Root cause:** `ModuleApiProxyTransformer.TransformRequestAsync` (`src/Core/DotNetCloud.Core.Server/Program.cs`) calls `base.TransformRequestAsync` — which **already copies every request header** except the hop-by-hop ones — and then adds **every** header a second time. `TryAddWithoutValidation` appends instead of replacing, so the module host receives two `If-None-Match` values and `Request.Headers.IfNoneMatch.ToString()` becomes `"…","…"`, which can never equal the token. The old code special-cased only `Authorization` / `X-Device-*`, which is why the duplication went unnoticed.
- **Blast radius:** every module REST endpoint that depends on conditional headers — chat alerts, **Files chunk `If-None-Match` dedup**, Bookmarks ETag. Silent degradation only (extra bandwidth/CPU on both sides), no data loss.
- **Fix:** skip any header already present on the proxy request (`proxyRequest.Headers.Contains(header.Key)`); headers the base skips (hop-by-hop) are still forwarded. The class was made `internal` so it is unit-testable.
- **Tests:** `tests/DotNetCloud.Core.Server.Tests/Proxy/ModuleApiProxyTransformerTests.cs` (4 tests) — **written first and failing** against the old code (`If-None-Match` count `expected 1, actual 2`), green after the fix. Core.Server suite **784 pass / 1 skipped**; the single failure is the pre-existing `ProgramRootCaTests.GetRootCaPath_AbsolutePath_ReturnsSiblingRootCa`.

**Done on `cloud`:** deployed the branch (`sudo ./scripts/deploy.sh --force --verify`) and re-ran the two-leg check:

```bash
curl -sS -D- -o- -H "Authorization: Bearer $TOKEN" https://cloud.dotnetcloud.net/api/v1/chat/alerts
# → 200 + the { success, data } envelope + an ETag header

curl -sS -D- -o- -H "Authorization: Bearer $TOKEN" \
     -H 'If-None-Match: "<the etag above>"' https://cloud.dotnetcloud.net/api/v1/chat/alerts
# → 304, empty body
```

**✅ DONE (client agent — `monolith`, 2026-09-19, phone R5CWC356B2K): the `304` is confirmed from the device itself, and the changed path is regression-checked.**

| Check | Evidence |
| --- | --- |
| Job armed | job **3108** `PERSISTED`, **any** network (not the media job's unmetered `3107`), min latency 60 s / max delay 120 s while unread |
...
| Read clears it | **0** notification records for the package afterwards; the next forced poll returned `304` (`UpToDate`) — **no second notification** |
| No foreground service | `dumpsys activity services net.dotnetcloud.client` → `(nothing)` |

Recipe (kept for reuse — executed on `R5CWC356B2K` 2026-09-19):

1. Confirm the persisted job is still armed (use `am kill`, **never** `am force-stop` — it cancels the job):
   `adb shell dumpsys jobscheduler | grep -A11 '/3108:'` → job **3108** `PERSISTED`
2. With **no** chat activity since the last poll, force one run:
   `adb shell cmd jobscheduler run -f net.dotnetcloud.client 3108`
3. Watch the poll output:
   `adb logcat -s DotNetCloud`
   - ✅ Expected: `Chat alert poll: aggregate unchanged` (`ChatAlertPollOutcome.UpToDate`) — a `304`, and the aggregate body was **not** read.
   - ❌ Regression: the poll takes the full-read path for unchanged state, or the request is answered `200` with a body.
4. ✅ Regression-checked: a message to a **non-muted** channel while the app was closed produced `- 200` → `finished (Alerted, pending: True)` and **one generic notification** (title "New message", empty body) that deep-linked to exactly its channel; it was cleared on read with **no** second notification. The `200` leg needs a message from **another user** (a moderator action from the web client) — a same-user message cannot create unread, so it cannot exercise the alert path.
...
- **The client wake path AND the alert itself are verified on-device** — see the results table above (2026-09-19, R5CWC356B2K). The pre-deploy `404` is gone; the remaining gap is only the `304` optimization, and its cause is the proxy duplication described above (not the Chat endpoint).
- **Whenever `posted` must be observed** (poll-only alert with the app closed), note that the *in-app* SignalR path and the Doze alarm both post through the same notifier: if the process is still alive when a message arrives, one of them wins and the poll then correctly logs `Suppressed`. Kill the process (`adb shell am kill net.dotnetcloud.client`) instead of `am force-stop` — **`am force-stop` cancels the persisted job**, and `am kill` leaves it armed (`dumpsys jobscheduler | grep -A11 '/3108:'`).
- The abandoned push connector was **deleted outright** in this branch: do not be surprised that the app no longer contains a receiver/registration state machine. The notification payload contract + generic renderer remain and are used by both the live SignalR path and the poll.
- The idle poll is **three cheap constant queries** and **zero aggregate queries** (not literally zero queries) — stated accurately in the plan so nobody is surprised by the DB cost.
...
**Status:** completed ✅ (2026-09-19, server agent — `cloud`) — deployed and verified. **The Android on-device E2E was completed by the client agent (`monolith`), and the follow-up core-proxy `304` fix is archived above — nothing outstanding.**
**Branch:** the chat-alerts feature branch · **Target:** core server + the Chat module host on `cloud.kimball.home`. **No extra DNS record, certificate entry, additional service or proxy rule was involved.**
...
- ⚠️ **No separate push infrastructure was needed**: no push transport implementation, no device-registration persistence, no extra DNS record or certificate, no proxy route.

### Original deploy steps (executed 2026-09-19)

1. Pull the branch used for the chat-alerts work.
...
1. **Android receiver/service `Name` bug → cold-start crash.** `CalendarBootReceiver`, `CalendarAlarmReceiver`, `FcmMessagingService` were declared both manually in `AndroidManifest.xml` (`.X` → `net.dotnetcloud.client.X`) and via `[BroadcastReceiver]`/`[Service]` attributes without explicit `Name`, so the generated Java class landed in a `crc…` package → `ClassNotFoundException` when Android instantiated it (the sticky `BOOT_COMPLETED` broadcast crashed every cold start). Fixed by adding `Name = "net.dotnetcloud.client.X"` to the attributes (mirrors the working `[Service(Name=…)]` pattern).
