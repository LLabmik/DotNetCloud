...
## Recent Work — Android Chat Alerts, background poll transport (2026-09-19)

> Tracked in `docs/ANDROID_CHAT_BACKGROUND_ALERTS_PLAN.md`. The app's own poll of `GET /api/v1/chat/alerts` is the whole transport: no companion app, no third-party push service, no Google, no foreground service.
...
- ✓ **Abandoned push connector pruned** (operator decision) — receiver, registration state machine, endpoint registrar, distributor picker, Settings card, intent helpers, connector foreground service and link activity, plus their tests and manifest entries. Kept only the payload contract + generic renderer, which both live notification paths use.
...
## Recent Work — Push-route removal & transport-neutral rename (2026-09-20, `chore/remove-abandoned-push-route`)

> Completes the 2026-09-19 prune: the abandoned third-party push route is now gone from code, config, docs and the test phone — no vendor name remains anywhere in the repository.

- ✓ **Chat module** — deleted the non-FCM push provider, its transport abstraction, its logging transport and its options class; `PushProvider` is now FCM-only; `DeviceRegistration.Endpoint` and the device-registration `endpoint` field removed from the Chat DTO, host controller and proto comment.
- ✓ **Core / UI** — `PushNotificationChannel` doc comment and `UserDevice.PushToken` docs corrected; the stale third-party push licence entry removed; the vendor section removed from the admin `PushNotificationSettings` page together with its now-unused `GetInt` helper.
- ✓ **Android** — payload contract renamed to `Services/NotificationPayloadContract.cs` (`NotificationPayloadContract`, `NotificationPayload`, `NotificationPlan`, `NotificationKind`, `NotificationTarget`) and the renderer to `Platforms/Android/ChatNotificationRenderer.cs`; the dead connector API (spec actions, intent builders, token/truncate helpers, `MaxMessageBytes`) was deleted with it. Callers updated: `ChatAlertNotifier`, `SignalRChatClient`, `MainActivity`, `ChatAlertPollDecision`, `ChatAlertsSummary`.
- ✓ **Tests** — the protocol test file became `NotificationPayloadContractTests` (connector tests dropped; payload, generic-text and hostile-payload tests kept), the provider test file deleted, `NotificationRouterTests` re-based on FCM-only multi-device routing, cadence/poller/poll-decision assertions repointed.
- ✓ **Docs** — design doc rewritten and renamed to `docs/ANDROID_CHAT_BACKGROUND_ALERTS_PLAN.md` (the previously superseded plan doc is deleted); the abandoned server spec removed from the mediation handoff and archive; chat-module, client, user and tracking docs purged of vendor names.
- ✓ **Device + workbench** — the leftover third-party distributor app was uninstalled from `R5CWC356B2K` (verified: no distributor package remains) and the local test rig deleted from `%TEMP%`; chat alert job **3108** still `PERSISTED`, so alerts are unaffected.
...
## Recent Work — Module proxy content-header regression (2026-09-21, `fix/module-proxy-content-headers`)

> Reported by the operator immediately after the 2026-09-19 deploy: **Collabora documents would not open and Notes would not save** — an operator reboot did not help (it is a code defect, not a runtime state issue).

- ✓ **Root cause located in the core module proxy** — the 2026-09-19 duplicate-header fix (`f019f027`) probed with `HttpRequestHeaders.Contains(header.Key)`. `Contains` **throws** `InvalidOperationException: Misused header name, 'Content-Type'` for headers that are not valid on an outgoing _request_ message (content headers belong on `proxyRequest.Content.Headers`). YARP reports that throw as `ForwarderError.RequestCreation`, so the gateway answered **502 Bad Gateway**. The code before that commit used `TryAddWithoutValidation`, which _returns `false`_ for those headers instead of throwing — which is why the regression landed exactly on that commit.
- ✓ **Blast radius** — only requests carrying `Content-Type` / `Content-Length`, i.e. every proxied **write**: `POST /api/v1/notes` (Notes save), `POST /api/v1/wopi/token/…` (Collabora/WOPI) and `POST /api/v1/files/upload/initiate` (file uploads — unreported by the operator but failing 20× in the log). All proxied **GET** requests were unaffected, which is why browsing and reading still worked.
- ✓ **Fix** — a `HasRequestHeader` helper **enumerates** the header collection (enumeration performs no name validation, so it cannot throw) instead of calling `Contains`; content headers keep being skipped via `TryAddWithoutValidation`'s `false` return. The reason `Contains` must never be used there is documented on the helper.
- ✓ **Tests** — `tests/DotNetCloud.Core.Server.Tests/Proxy/ModuleApiProxyTransformerTests.cs` gained 3 tests **written failing-first** against a `POST` carrying `Content-Type` / `Content-Length`; they reproduced the exact production exception (`Misused header name, 'Content-Type'`) before the fix and pass after it. Core.Server suite: **787 pass / 0 fail / 2 skip**.
- ✓ **Deployed + verified live (2026-09-21, server agent — `cloud`)** — `deploy.sh --force --verify` → **15/15 targets** with every module-host hash verified; `/health/ready` **Healthy 14/14 modules**; `blazor.web.js` **200**; no pending migrations. Gateway proof with unauthenticated probes — **before**: `POST /api/v1/notes` → **502**, `POST /api/v1/files/upload/initiate` → **502**, `GET /api/v1/notes` → `401`; **after**: both POSTs → **401** (auth challenge, i.e. request creation now succeeds) and the GET control → `401`. Journal since the restart: **0** `Misused header name` / `RequestCreation` / `Module API proxy failure` lines (28 of each in the 35 minutes following the reboot).
- ✓ **Operator acceptance confirmed (2026-09-21)** — against the deployed build the operator verified **Collabora document opening, Notes saving and file upload** in the browser. **Nothing outstanding — all three previously broken write paths are restored.**

## Recent Work — Home widget settings: scoped DbContext concurrency (2026-09-21, `fix/home-widget-dbcontext-concurrency`)

> Found while investigating the `Failed to load home widget preferences` error that appeared exactly once on each of the two boots preceding this fix.

- ✓ **Root cause** — `IUserSettingsService` is registered **scoped**, but the Blazor circuit that resolves it is long-lived, so one `UserSettingsService` — and the single transient `CoreDbContext` it captured — was shared by every settings consumer on that circuit: `MainLayout` (restoring the collapsed sidebar), `Home` (loading the widget layout), `GlobalMusicPlaybar` and the Music/Video/Photos/Chat pages. Blazor interleaves those components' initializers (the parent layout's query is still in flight when the child page starts its own), so EF threw `InvalidOperationException: A second operation was started on this context instance`. The deliberate `ServiceLifetime.Transient` registration of `CoreDbContext` (`e83ec41b`, "DbContext threading") could not prevent it: a scoped wrapper captures one instance for the whole scope.
- ✓ **Impact** — caught and logged, so non-fatal, but the home page silently fell back to the **default** widget layout/style for that circuit. It reproduced only on a cold connection pool, which is why it fired exactly once per boot (the first page load after a restart) and never again in that session.
- ✓ **Fix** — `UserSettingsService` now creates a short-lived context per operation from the existing custom `IDbContextFactory` (`DefaultDbContextFactory`), mirroring the Chat module's `DbNotificationPreferenceStore`. Nothing shares a context across awaits, so interleaving cannot collide; both methods specify tracking explicitly, so behaviour is unchanged.
- ✓ **Tests** — `UserSettingsServiceTests` gained 3 contract tests (one context per operation; 8 overlapping calls → 8 distinct contexts; the context is released after the call). `Core.Auth.Tests` 174 pass.
- ✓ **Deployed + verified live (2026-09-21, server agent — `cloud`)** — `deploy.sh --force --verify` → **15/15 targets** with hashes verified, deployed `DotNetCloud.Core.Auth.dll` **md5-identical** to the build output (`459304b7fb8cf6fbe35555be00a3cab4`), `/health/ready` **Healthy 14/14 modules**, `blazor.web.js` **200**, no pending migrations. Release build 0 warnings / 0 errors; Core.Tests 532, Core.Server.Tests 787 pass / 2 skip, Integration.Tests 183 pass / 13 skip. Post-deploy evidence: **9 home-page requests** served and **0** `second operation` / `Failed to load home widget preferences` / `InvalidOperationException` lines in the whole boot — each of the two previous boots logged 1 of each.
- ☐ **Wider audit not in scope** — any other scoped service that captures a `CoreDbContext` and is called from interleaving components carries the same latent hazard. The render-time surface was narrow (only `IUserSettingsService` is injected into components) and is now fixed; the remaining scoped Core services (`NotificationService`, `AuditLogService`, `Admin*`, the Auth capability services) are untouched.

## Recent Work — Scoped DbContext sweep: directory + admin settings services (2026-09-22, `fix/scoped-dbcontext-sweep`)

> Completes the sweep the home-widget fix deliberately left open. All 25 services that capture a `CoreDbContext` in their constructor were audited, and the ones a long-lived holder can reach were converted.

- ✓ **Audit result — most of the family is already safe.** No captive dependencies anywhere: every singleton was checked, and `AdminSharedFolderCleanupService` / `SearchReindexHostedService` resolve the context inside a **fresh scope per use**, `PresenceService` already takes `IDbContextFactory`, `PresenceActivityMonitor` uses `IServiceScopeFactory`, and the singleton `GrpcHealthServiceImpl` (core capabilities, called by all 14 module hosts) creates a scope for every scoped lookup. Controllers, gRPC services, hosted services, the search providers, `DbInitializer`, the seeders and migrations all get a fresh scope per request/iteration, so a captured context lives for one operation only.
- ✓ **Converted — the render-time surface:** `UserDirectoryService`, `TeamDirectoryService`, `OrganizationDirectoryService`, `GroupDirectoryService` and `AdminSettingsService` now create a short-lived context per operation from `IDbContextFactory`, with the context threaded into their private helpers (and `OrganizationDirectoryService`'s duplicated membership read extracted into `FindMemberAsync`). These are the DB-backed implementations the Blazor host resolves — the gRPC variants are module-host-only — so circuits holding them carried exactly the home-widget hazard.
- ✓ **Tests** — new `DirectoryServiceContextTests` (team / organization / group) plus contract tests in `UserDirectoryServiceTests` and `AdminSettingsServiceTests`, all reworked onto a shared `InMemoryCoreDbContextFactory` double that records every context it hands out. `Core.Auth.Tests` **179 pass** (was 174).
- ✓ **REG-1 found + fixed in this branch — module-host DI gap.** The first sweep deploy broke the **Calendar and AI module hosts**: `Unable to resolve service for type 'DotNetCloud.Core.Data.Context.IDbContextFactory' while attempting to activate '...OrganizationDirectoryService'`, 4× in 30s. Process-isolated hosts build their **own** container, so they never inherited the factory from Core.Server's `AddDotNetCloudDbContext` — while they do register a DB-backed converted service next to their own `AddDbContext<CoreDbContext>`. Fixed with a single-source-of-truth helper `AddCoreDbContextFactory(connectionString, provider)` (now also used by `AddDotNetCloudDbContext`) called beside every raw `AddDbContext<CoreDbContext>`: `Calendar.Host`, `AI.Host`, and `Files.Host` pre-emptively. Its XML doc states the rule and the exact failure message.
...
5. Android MAUI app (chat, generic background alerts)
6. Push notifications (FCM) and the app's own background alert poll
...
- [ ] Generic background alerts reach Android devices (app's own poll; no push transport involved)
...
**FCM push support for mobile clients, plus the app's own generic background alerts**
...
- ✓ Create `DeviceRegistration` model:
  ...
  - ✓ `PushProvider Provider` property (FCM)
- ✓ Create `PushProvider` enum (FCM)
- ✓ Create `NotificationCategory` enum (ChatMessage, ChatMention, Announcement, FileShared, System)

#### Abandoned third-party push route — ❌ REMOVED (declined 2026-09-18; fully removed from the repository 2026-09-20)

> **Nothing here is pending.** The route that required every user to install and configure a separate push
> distributor app was declined by the operator, and has since been **deleted** — pipeline, provider, options,
> admin settings UI, tests, docs and the phone-side app. Background alerts ship as the app's own poll
...
#### Push Provider — ❌ REMOVED (2026-09-20)

> The second (non-FCM) push provider, its transport abstraction and its options class were deleted, together
> with the `endpoint` field on the device-registration payload. `NotificationRouter` now routes by device to
> the FCM provider only. Nothing is pending here; see the 2026-09-20 entry in "Recent Work".
...
- ✓ Set up build flavors: `googleplay` and `fdroid` — both share the same alert code path and differ by application id only
...
- ✓ Background connection management — `ChatConnectionService` is a sticky but **unpromoted** service: the `dataSync` foreground service was removed outright on 2026-09-12 because Android 15/16 caps `dataSync` to a rolling 24-hour budget and the resulting kill loop was structural, not tunable. Background alerts do not come from this service: `ChatAlertJobService` (job 3108) polls the server while the app is closed.
- ✓ Handle Doze mode and battery optimization

#### Push Notifications

- ✓ Integrate Firebase Cloud Messaging (FCM) for `googleplay` flavor
  - **Superseded (2026-09-18):** this is **no longer a pending task**. FCM never worked in this build (no `google-services.json` → `FirebaseApp` never initialises → no token), and the operator has decided FCM is **removed outright** rather than configured — no fallback, no behind-a-flag build. Do not create a Firebase project; alerts come from the app's own background poll.
- ✓ Integrate the second (non-FCM) push transport for the `fdroid` flavour — ❌ **removed 2026-09-20** (see above).
...
- ✓ Debug native-crash config merged and released (2026-09-16, PR #143 → `62f26f85`, v0.6.09): the merged change also sets `AndroidEnablePreloadAssemblies=true` (assemblies preloaded up front on the startup thread rather than on demand from the first thread that needs them) and documents the full crash chain above the property. Rebased onto `main` and conflict-resolved against the already-merged interpreter line; post-rebase Debug arm64 build 0 warnings / 0 errors, Android tests 334 passed / 1 skipped, shipped APK `versionName 0.6.09` / `versionCode 16`. ⚠️ Preload is **not** individually proven — a preload-only run with the interpreter still on did not stop the crashes (2 of 9 forced headless starts), so its value with the interpreter off is unproven; it costs ~3 s of Debug startup and more RSS, so revisit (soak both variants or drop it) if Debug cold-start memory matters
...
- ✓ F-Droid flavor build + background-alert smoke test — **RESOLVED 2026-09-18**: the `fdroid` flavour builds (0 warnings) and is verified on device; the abandoned push dependency was dropped (the pinned package never existed on nuget.org). **Superseded 2026-09-20:** both flavours now use the same poll-based alert path, so there is no per-flavour push difference to test.
...
- ✓ **Background alerts have a working transport (2026-09-19):** `ChatAlertJobService` (job **3108**) polls `GET /api/v1/chat/alerts` while the app is closed, with `ChatAlertAlarmReceiver` covering Doze and the in-app SignalR path covering the foreground. Delivered, deployed and live-verified — see the Android Chat Alerts entry above. (FCM is **not** the answer: the client ships no Google messaging dependency.)
...
- ✓ `PushNotificationChannel` — gRPC to Chat module `NotificationRouter` (FCM)
