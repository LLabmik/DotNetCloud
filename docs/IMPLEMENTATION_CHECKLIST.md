...
- ☐ **Phone-side `304` confirmation — deferred to client agent (`monolith`)** — `cloud` has no ADB/phone attached, so this on-device check goes back to `monolith`: with no chat activity since the last poll, `adb shell cmd jobscheduler run -f net.dotnetcloud.client 3108` should log `Chat alert poll: aggregate unchanged` (`UpToDate` / `304`, i.e. the aggregate body is not read); also regression-check that a message to a non-muted channel still alerts while the app is closed. Use `am kill`, **not** `am force-stop` (which cancels job 3108). Not a blocker — the server contract is already proven live.
