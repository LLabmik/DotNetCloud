...
- ✓ `Controls/MarkdownWebView.cs` — 2026-09-17: control now hosts its own conversion spinner and its auto-height actually works. `Navigated` never fires for inline HTML (MAUI suppresses it for `about:blank` sources), so the old measurement was dead: finished answers were clipped to the 88dp placeholder. The control now tags each document with a render token and probes `painted:height` (painting detected with a double `requestAnimationFrame`), sizing the bubble to its content and holding the spinner for at least 350 ms. `MarkdownHtmlFormatter.ToHtmlDocument()` gained the optional `documentToken` parameter; `EventEditPage` preview uses the new `Refresh()` (a render that happens while hidden can neither paint nor be measured).
...
**Notes:** Android AI tab now renders full Markdown via client-side Markdig → themed HTML → auto-height WebView, reusing the Notes WebView pattern. Streaming remains on the lightweight `FormattedString` renderer for performance. The AI stream watchdog was fixed so a cold Ollama model load is never cancelled before the first token arrives (only armed after first content/thinking). **Answer-conversion fix (2026-09-17, on-device verified):** a completed answer switched from the streaming bubble to a WebView bubble that (a) was clipped to the 88dp placeholder height and (b) showed a blank surface while the WebView converted the Markdown — the operator reported it as "a blank space that makes the user think something has gone wrong". Both are fixed in `MarkdownWebView` (render probe → real height + built-in spinner). **Blank answer area during reasoning (2026-09-17, on-device verified):** the remaining blank the operator hit is the *streaming* bubble's answer area — `IsModelLoading` (the "Generating…" indicator) also required the thinking text to be empty, so a reasoning model (gemma4:12b writes a plan first) suppressed the indicator for the entire reasoning phase and left a 320dp empty bubble under the thought process. The indicator is now keyed on the answer text only (`string.IsNullOrEmpty(StreamingContent)`), re-checked on the UI thread so a late token can't re-show it. **Verification complete 2026-09-01:** Android arm64 build clean (0 warnings), 257 unit tests pass, and on-device E2E on Samsung R5CWC356B2K confirmed the app waits through the slow first token (cold model load, incl. a ~2-min server-side network stall caused by phone USB-tethering multihoming) with no 60s "AI stream timed out" error, then streams the reply. **Verification 2026-09-17:** arm64 Debug build clean (0 warnings), Android tests 347 pass / 1 skip, and on-device (R5CWC356B2K) a follow-up answer showed the spinner over the still-converting bubble (confirmed by pixel-level frame inspection) and then the complete, correctly sized answer; the calendar event-description preview (same control) renders and auto-sizes too; the reasoning phase now shows a spinner + "Generating…" where the blank bubble used to be.
