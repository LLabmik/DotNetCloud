...
**Android folder picker (2026-09-15, on-device verification still owed):** The Android half is implemented and committed on `fix/notes-folder-assignment` **without the on-device E2E** — committed at the operator's explicit instruction (the no-commit-until-verified gate was knowingly waived; the emulator route was attempted but could not complete a sign-in to `cloud.dotnetcloud.net`, see the handoff doc for the `invalid-credentials` analysis). `NoteEditViewModel` loads the caller's folders in create and edit mode (`FolderOptions`, leading "None (unfiled)" entry), sends `UpdateNoteDto.ClearFolder = true` when a note is moved to "None" (the REST/gRPC contract is already live server-side), keeps a shared note's owner folder visible as "Shared folder" so a save cannot silently unfile it, and hides the picker for a non-owner; the notes list passes the active folder chip into the editor and tags each card with the folder name in small red text. 325 Android tests pass / 1 skip; arm64 Debug build 0 warnings / 0 errors. Remaining: on-device E2E (create-in-folder, move, unfile-confirmed-via-server-reload) then commit + push.
