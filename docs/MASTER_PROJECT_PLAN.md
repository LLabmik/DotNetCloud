...
> **Recent fix (2026-09-17):** Android chat alert sound. Incoming messages were silent with the app on screen — notifications are suppressed in the foreground by design and the web client's in-app ding had never been ported — so `IChatSoundPlayer`/`AndroidChatSoundPlayer` (SoundPool, `USAGE_NOTIFICATION`) now plays the shared `chat-ding.mp3`, with `ChatAlertPolicy` guaranteeing exactly one alert per message (ding in the foreground, notification in the background, nothing for muted channels or own-message echoes) and a "Message Sound" switch under Settings → Chat Notifications. Verified live on-device with a real remote message (`decision=InAppSound` + audio `event:started`), 10 new unit tests. **Background alerts remain blocked on a push transport:** the googleplay build ships without `google-services.json` so Firebase never initialises and no FCM token can be issued; push device registration is now re-sent on every launch and after login (the server stores registrations in memory only), so delivery starts working as soon as a Firebase project is added. ⚠️ **Operator decision (same day): Google must be out of the push path entirely** (no content _and_ no metadata), so the transport of record becomes **UnifiedPush + self-hosted ntfy** with completely generic notifications — plan (not yet implemented): `docs/ANDROID_UNIFIEDPUSH_PLAN.md`.

> **Recent feature work (2026-09-18):** **UnifiedPush push (privacy-first) — Android half SHIPPED.** The `fdroid` flavour builds again (the blocker was that `UnifiedPush.NET` **does not exist on nuget.org**) and now registers with a user-chosen distributor, receives messages while backgrounded **and force-stopped**, and renders **completely generic** notifications (no sender, channel name or text). FCM is **deleted outright** from both flavours (no fallback, no flag), and Settings shows the distributor + registration state with a "Choose distributor" action. Verified on device: register → endpoint → delivery → tap routing, the spec's `RAISE_TO_FOREGROUND` cold start, and 425 unit tests green. ⚠️ A device test answered the plan's first open question — **the distributor rejects a push server URL containing a path** — so the server half must expose ntfy by **Host** (`push.<domain>`) on the same 443 instead of a `/push` path route; that deviates from requirement §1.5 ("no extra hostname") and needs the operator's decision (plan §9.10).

> **Handoff (2026-09-18):** **UnifiedPush push transport (privacy-first)** — the **server half** is spec'd and **deferred** (operator, 2026-09-18 — nothing authorised to start): real `UnifiedPushHttpTransport`, persisted device registrations, ID-only payloads, a chat-message push builder, the presence-suppression fix, and the **Host-routed loopback ntfy** exposure (no new port or firewall rule; ⚠️ now needs a DNS record + certificate entry for the push hostname, see plan §9.10). Plan/spec `docs/ANDROID_UNIFIEDPUSH_PLAN.md` (§5 Phase 3 acceptance, §7 topology, §8 server spec), deferred handoff in `docs/development/CLIENT_SERVER_MEDIATION_HANDOFF.md`. **The Android half is complete and on-device verified** (plan phases 1, 2 + the client-side of phase 5).

> **Planning (2026-09-19):** **Android push — "our code only" poll tiers ✅ ADOPTED (plan §12).** Because the UnifiedPush distributor install is unacceptable and FCM/Google is ruled out, the operator accepted a **best-effort latency budget** (seconds while the phone is in use, ~1–5 min screen-off, ~9 min in deep Doze) and therefore adopted **Tiers 1–2** as the transport of record: (1) a `JobScheduler` poll of the **already existing** `GET /api/v1/chat/unread` — which already returns per-channel `UnreadCount`/`MentionCount`/`IsMuted` — plus immediate polls on screen-on/unlock/connectivity; (2) an exact-alarm Doze path (~9 min floor) reusing the existing `IExactAlarmPermissionService`. **Tier 3 (an opt-in `specialUse` foreground service) is DECLINED**, so the 2026-09-12 "no chat FGS" rule stands, no persistent notification is added, and both flavours behave identically. The single server-side item (a cheap aggregate `chat/alerts` endpoint + `ETag`) is **approved in principle but not yet promoted** to an Active Handoff, because the current unread endpoint costs 2 queries per channel membership per poll. ✅ **Implemented 2026-09-19 (both halves)** — plan §12.10 "As built"; server tests 1431 pass, Android tests 478 pass / 1 skip, arm64 Debug build 0 warnings. ✅ **Server half deployed + verified** on `cloud.kimball.home` (15/15 deploy targets, `/health/ready` Healthy 14/14, route proof `401` vs control `404`). ✅ **Live E2E PASSED on device (2026-09-19, R5CWC356B2K, client agent — `monolith`):** the poll answers `200` with the app's own token; with the app process **dead** a new message produced `finished (Alerted, pending: True)` (`posted=True`, `foreground=False`) and **one generic notification** (title "New message", empty body, no sender/channel) whose tap opened exactly its channel; reading it cleared the alert without a second notification; a **muted** DM produced nothing; **no foreground service** (job **3108** `PERSISTED`, any-network, self-re-arming 60 s/300 s). ✅ **Closed (2026-09-19):** `If-None-Match` → `304` previously never fired because `ModuleApiProxyTransformer` (`Core.Server/Program.cs`) forwards every request header **twice** (a module host sees `"…","…"`, which can never match; affects every conditional-header endpoint reaching a module, incl. Files chunk dedup) — **fixed + unit-tested on `monolith`**, then **deployed to `cloud` and re-verified live: leg 1 `200` + `ETag`, leg 2 `304` with an empty body** (15/15 deploy targets, `/health/ready` Healthy 14/14, deployed core DLL md5-identical to the build output, `ModuleApiProxyTransformerTests` 4/4). **Server side closed — nothing outstanding.** ✅ **Client-side confirmation DONE (2026-09-19, `monolith`, phone R5CWC356B2K):** the phone's own poll receives the **`304`** on the wire (`Received HTTP response headers … - 304`, `finished (UpToDate, pending: True)`) with no aggregate body read, and the changed path still alerts — a message into a non-muted channel with the app process **dead** returned **`200`** → `finished (Alerted, pending: True)` → **one generic notification** (`chat_messages`, title "New message", empty body) that deep-linked to exactly its channel; opening the channel cleared it (0 notification records for the package, next poll `304`, no second notification) and `dumpsys activity services` stayed empty. **The chat-alerts poll contract is verified end-to-end in both directions — nothing outstanding.**
...
- ✓ Push: FcmMessagingService (googleplay flavor), UnifiedPushReceiver (fdroid flavor), 5 notification channels (connection, messages, mentions, announcements, photo_upload), AndroidManifest declarations
  - **Superseded 2026-09-18:** the FCM half was **deleted outright** (no fallback, no flag) and the `UnifiedPushReceiver` was rewritten as a plain `BroadcastReceiver` implementing spec AND_3.1.0 — see the 2026-09-18 note at the top and `docs/ANDROID_UNIFIEDPUSH_PLAN.md`.
