...
- ✓ Tray context menu log access actions: "Open sync service logs" and "Open tray logs"
...
**Blocking Issues:** None
**Notes:** Phase 1.16 complete. Full solution builds cleanly for SyncTray, and focused SyncTray IPC/settings regression tests pass (`dotnet test ... --filter "FullyQualifiedName~IpcClientTests|FullyQualifiedName~SettingsViewModelTests"` -> 18/18). Tray icons are programmatic coloured circles (placeholder — production icons should be added to `Assets/`). Linux graphical-login auto-start is implemented via an XDG autostart desktop entry, and SyncTray bootstraps SyncService automatically when launched in a Linux desktop session. A duplicate `list-contexts` startup race in the tray IPC client was fixed so self-started Linux sessions no longer fall back to a false "no accounts configured" state when concurrent callers query account state during service bring-up. Startup now also waits for the Linux IPC socket to appear before entering the reconnect loop, removing the expected first-launch socket race warning in normal self-start scenarios. SyncTray startup ensures a per-user Linux application-menu launcher (`~/.local/share/applications/dotnetcloud-sync-tray.desktop`) and now sets a cloud-themed launcher icon from bundled desktop assets (`Assets/dotnetcloud-sync-cloud.svg`). Tray menu also includes direct log access for both sync-service and tray logs to speed up troubleshooting from the system tray. Bandwidth-limit enforcement remains deferred (UI is present, enforcement in the sync engine is a future enhancement).
...
**Tasks:**
- ✓ Add chat notification popups with message preview and channel-aware titles (current implementation: Windows toast + Linux libnotify)
...
- ✓ Migrate Windows notifications from balloon tips to Windows toast notifications
