...
> **Handoff (2026-09-18):** **UnifiedPush push transport (privacy-first)** — Android background alerts without Google. Operator decision 2026-09-17: Google is out of the push path entirely (no content _and_ no metadata), so the transport of record is **UnifiedPush with a self-hosted ntfy** and completely generic notifications. The **server half** (real `UnifiedPushHttpTransport`, persisted device registrations, ID-only payloads, a chat-message push builder, presence-suppression fix, and the in-app `/push` proxy route that lets ntfy run on **loopback behind the instance's own port** — no new port, certificate, hostname or firewall rule) is **spec'd and deferred** (operator, 2026-09-18 — nothing authorised to start): plan/spec `docs/ANDROID_UNIFIEDPUSH_PLAN.md` (§5 Phase 3 acceptance, §7 topology matrix, §8 server spec), deferred handoff in `docs/development/CLIENT_SERVER_MEDIATION_HANDOFF.md`. The Android half (fdroid flavour does not compile — `UnifiedPush.NET` 2.0.2 does not exist on nuget.org — plus a hand-written UP connector) remains with the client agent. Nothing implemented yet.
