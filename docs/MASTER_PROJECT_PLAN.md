...
- ☐ **Trash restore loses the directory structure (server-side defect, handed off).** `TrashService.RestoreAsync` resolves the original parent without `IgnoreQueryFilters()`, so a parent that is itself still trashed is not found and the node is restored to the **root**; `RestoreAllAsync` selects `OriginalParentId != null` — exactly the descendants — and therefore skips the deleted folder itself and restores each child to root. Encountered live when restoring a bug-deleted `AutoUpload` folder (2026-09-15).
