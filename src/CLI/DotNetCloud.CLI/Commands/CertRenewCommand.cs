...
internal static class CertRenewCommand
{
    ...
    /// <summary>
    /// Writes the Collabora environment file consumed by the systemd unit's EnvironmentFile directive.
    /// Generates /etc/dotnetcloud/env with all Files__Collabora__* variables needed at runtime.
    /// </summary>
    internal static void WriteCollaboraEnvFile(CliConfig config)
    {
        ...
        try
        {
            ...
            ConsoleOutput.WriteSuccess($"Collabora env file written to {envPath}.");
        }
        catch (Exception ex)
        {
            ConsoleOutput.WriteWarning($"Failed to write Collabora env file: {ex.Message}");
        }
    }
