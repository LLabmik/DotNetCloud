...
namespace DotNetCloud.Core.Search.Extraction;
...
public sealed class ContentExtractionService
{
    ...
    /// <summary>
    /// Extracts text content from a byte array using the appropriate extractor.
    /// Entry point used by the out-of-process extraction worker's gRPC service.
    /// </summary>
    /// <param name="content">The document content as a byte array.</param>
    /// <param name="mimeType">The MIME type of the document.</param>
    /// <param name="cancellationToken">A token to cancel the operation.</param>
    /// <returns>The extracted content, or null if no extractor supports the MIME type or extraction fails.</returns>
    public async Task<ExtractedContent?> ExtractAsync(byte[] content, string mimeType, CancellationToken cancellationToken = default)
    {
        ...
        return await ExtractAsync(ms, mimeType, cancellationToken);
    }
