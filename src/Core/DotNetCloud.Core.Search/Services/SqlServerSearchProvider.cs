...
namespace DotNetCloud.Core.Search.Services;

/// <summary>
/// SQL Server full-text search provider using <c>FREETEXT</c>/<c>FREETEXTTABLE</c> for native FTS.
/// Falls back to <c>Contains</c> when running on non-SQL Server providers (e.g., InMemory for tests).
...
public sealed class SqlServerSearchProvider : ISearchProvider
{
    ...
    public SqlServerSearchProvider(CoreDbContext db, ILogger<SqlServerSearchProvider> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    public async Task IndexDocumentAsync(SearchDocument document, CancellationToken cancellationToken = default)
    {
        ...
        // CoreDbContext uses the SqlServerRetryingExecutionStrategy, which does not
        // support user-initiated transactions started outside the strategy. Wrap the
        // transactional upsert in the execution strategy so EF can retry it as a unit.
    ...
    public async Task<SearchResultDto> SearchAsync(SearchQuery query, CancellationToken cancellationToken = default)
    {
        ...
        // SQL Server FREETEXT/CONTAINS fallback to Contains() for InMemory
    ...
    /// <summary>
    /// Upserts a batch of documents using batched <c>SaveChanges</c> calls for efficiency
    /// during full reindexes. Loads existing entries for the module once, then updates or
    /// inserts accordingly (one DB round-trip per batch instead of per document).
    /// </summary>
    /// <param name="documents">The documents to index (all from the same module).</param>
    /// <param name="cancellationToken">A token to cancel the operation.</param>
    /// <returns>The number of documents indexed.</returns>
    public async Task<int> BatchIndexAsync(IReadOnlyList<SearchDocument> documents, CancellationToken cancellationToken = default)
    {
        if (documents.Count == 0)
            return 0;

        const int batchSize = 200;
        var moduleId = documents[0].ModuleId;

        // Load existing entries for this module once so updates don't require a query per document.
        var existing = await _db.SearchIndexEntries
            .Where(e => e.ModuleId == moduleId)
            .ToListAsync(cancellationToken);
        var existingMap = existing.ToDictionary(e => e.EntityId);

        var count = 0;
        foreach (var document in documents)
        {
            if (existingMap.TryGetValue(document.EntityId, out var entry))
            {
                entry.EntityType = document.EntityType;
                entry.Title = document.Title;
                entry.Content = document.Content;
                entry.Summary = document.Summary;
                entry.OwnerId = document.OwnerId;
                entry.OrganizationId = document.OrganizationId;
                entry.CreatedAt = document.CreatedAt;
                entry.UpdatedAt = document.UpdatedAt;
                entry.IndexedAt = DateTimeOffset.UtcNow;
                entry.MetadataJson = document.Metadata.Count > 0
                    ? JsonSerializer.Serialize(document.Metadata)
                    : null;
            }
            else
