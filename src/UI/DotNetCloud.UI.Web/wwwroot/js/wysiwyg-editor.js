...
window.wysiwygEditor = {
  ...
  _handlePastedImage: async function (editor, item) {
    ...
    // Tell .NET the upload has started so the UI can show an "Uploading…" indicator.
    await this._reportPasteUploadState(editor, true);

    // Upload via HTTP to bypass SignalR message size limits
    if (editor.channelId && window.chatImageUpload?.uploadImageFile) {
      const result = await window.chatImageUpload.uploadImageFile(
        file,
        editor.channelId,
      );
      if (result) {
        ...
        // The .NET handler clears the uploading indicator once the thumbnail is added.
        return;
      }
    }
    ...
    if (!dataUrl) {
      // Nothing could be read/uploaded — stop the uploading indicator.
      await this._reportPasteUploadState(editor, false);
      return;
    }

    const forwarded = await editor.dotNetRef.invokeMethodAsync(
    ...
    if (!forwarded) {
      // Processing failed before an attachment could be created — stop the indicator.
      await this._reportPasteUploadState(editor, false);
    }
  },

  /**
   * Reports paste-upload activity (true = started, false = finished/failed) to .NET.
   * Returns false if the .NET side is no longer available (component disposed).
   * @param {object} editor
   * @param {boolean} isUploading
   * @returns {Promise<boolean>}
   */
  _reportPasteUploadState: async function (editor, isUploading) {
    try {
      await editor.dotNetRef.invokeMethodAsync(
        "HandlePasteImageUploadStateChanged",
        isUploading,
      );
      return true;
    } catch {
      // Component disposed mid-upload; nothing to update.
      return false;
    }
  },
  ...
  _toggleInlineCode: function (editorEl) {
    ...
    while (parent && parent !== editorEl) {
      if (
        parent.nodeName?.toLowerCase() === "code" &&
  ...
  _markdownToHtml: function (markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith("```")) {
        if (inCodeBlock) {
          html +=
            "<pre><code>" +
            this._escapeHtml(codeBlockContent) +
            "</code></pre>";
          codeBlockContent = "";
          inCodeBlock = false;
        } else {
      ...
      if (hMatch) {
        ...
        html +=
          "<h" + lvl + ">" + this._inlineFmt(hMatch[2]) + "</h" + lvl + ">";
        continue;
      }
      ...
      if (trimmed.startsWith("> ")) {
        ...
        html +=
          "<blockquote>" +
          this._inlineFmt(trimmed.substring(2)) +
          "</blockquote>";
        continue;
      }
    ...
    if (inCodeBlock)
      html +=
        "<pre><code>" + this._escapeHtml(codeBlockContent) + "</code></pre>";
