...
window.chatImageUpload = {
  ...
  _initListener: function (inputId, input, dotNetRef, channelId) {
    ...
    input.addEventListener("change", async () => {
      ...
      // Tell .NET an upload has started so the UI can show an "Uploading…" indicator.
      try {
        await ctx.dotNetRef.invokeMethodAsync(
          "HandleImageUploadStateChanged",
          true,
        );
      } catch {
      ...
      if (!result) {
        // Upload failed — stop the uploading indicator.
        try {
          await ctx.dotNetRef.invokeMethodAsync(
            "HandleImageUploadStateChanged",
            false,
      ...
      // HandleImageUploaded clears the uploading indicator once the thumbnail is added.
    });
  },
  ...
  uploadImageFile: async function (file, channelId) {
    ...
    try {
      const response = await fetch(
        `/api/v1/chat/channels/${channelId}/upload-image`,
        {
          method: "POST",
          headers: {
