...
window.dotnetcloudUpload = (function () {
  ...
  async function getMaxUploadSize() {
    ...
    try {
      const resp = await fetch("/api/v1/files/config", {
  ...
  async function uploadFile(fileIndex, userId, parentId, dotNetRef) {
    ...
    try {
      ...
      try {
        const quotaResp = await fetch("/api/v1/files/quota", {
          credentials: "same-origin",
        });
        if (quotaResp.ok) {
          ...
          if (q.maxBytes > 0 && file.size > q.remainingBytes) {
            await dotNetRef.invokeMethodAsync(
              "OnJsUploadError",
              fileIndex,
              "Insufficient storage quota. This file is " +
                formatSize(file.size) +
                " but you only have " +
                formatSize(q.remainingBytes) +
                " remaining" +
                " (used " +
                formatSize(q.usedBytes) +
                " of " +
                formatSize(q.maxBytes) +
                ").",
            );
            return;
          }
        ...
        /* quota check is best-effort; server will enforce anyway */
      }
      ...
      if (!sessionId) {
        ...
        const initResp = await fetch(`${apiBase}/upload/initiate`, {
          method: "POST",
        ...
        if (!initResp.ok) {
          ...
          try {
            ...
            const msg =
              (errJson.error && errJson.error.message) || errJson.message;
            if (msg) friendlyMsg = msg;
          } catch {
            /* not JSON, use raw */ friendlyMsg = errText || friendlyMsg;
          }
        ...
        missingSet = new Set(
          (session.missingChunks || []).map((h) => h.toLowerCase()),
        );
      } else {
        // Resume: query server for remaining missing chunks
        const statusResp = await fetch(`${apiBase}/upload/${sessionId}`, {
          credentials: "same-origin",
          signal: state.abortController.signal,
        });
        if (statusResp.ok) {
          ...
          missingSet = new Set(
            (statusData.missingChunks || []).map((h) => h.toLowerCase()),
          );
        } else {
      ...
      for (const chunk of chunks) {
        ...
        if (!putResp.ok) {
          ...
          throw new Error(
            `Chunk upload failed (${putResp.status}): ${errText}`,
          );
        }
  ...
  function addExternalFiles(files) {
    ...
    for (const f of files) {
      ...
      const relativePath = normalizeRelativePath(
        explicitPath || file.webkitRelativePath || null,
      );
  ...
  async function ensureFolder(parentId, folderName, userId, apiBase) {
    ...
    if (!createResponse.ok) {
      ...
      throw new Error(
        `Create folder failed (${createResponse.status}): ${errText}`,
      );
    }
  ...
  async function resolveTargetParentId(
    relativePath,
    rootParentId,
    userId,
    apiBase,
