...
/// Uses canonical UserVideo IDs throughout.
/// </summary>
public sealed class WatchProgressService : IWatchProgressService
{
    ...
    public async Task UpdateProgressAsync(Guid videoId, UpdateWatchProgressDto dto, CallerContext caller, CancellationToken cancellationToken = default)
    {
        ...
        var durationTicks = userVideo.CanonicalVideo?.DurationTicks ?? 0;
        if (durationTicks > 0)
        {
            progress.IsCompleted = (double)dto.PositionTicks / durationTicks >= 0.9;
        }
    ...
    public async Task<WatchProgressDto?> GetProgressAsync(Guid videoId, CallerContext caller, CancellationToken cancellationToken = default)
    {
        ...
        return await MapToDtoAsync(progress, cancellationToken);
    }
    ...
    public async Task<IReadOnlyList<WatchProgressDto>> GetContinueWatchingAsync(CallerContext caller, int count = 20, CancellationToken cancellationToken = default)
    {
        ...
        var results = new List<WatchProgressDto>();
        foreach (var progress in progresses)
        {
            var dto = await MapToDtoAsync(progress, cancellationToken);
            if (dto is not null)
                results.Add(dto);
        }
    ...
    /// <summary>
    /// Records a view (increments view count and publishes event).
    /// </summary>
    ...
    private async Task<WatchProgressDto?> MapToDtoAsync(WatchProgress progress, CancellationToken cancellationToken)
    {
        var userVideo = await _db.UserVideos
            .Include(uv => uv.CanonicalVideo)
            .FirstOrDefaultAsync(uv => uv.Id == progress.VideoId, cancellationToken);

        var canonicalVideo = userVideo?.CanonicalVideo;
        var durationTicks = canonicalVideo?.DurationTicks ?? 0;
        ...
        return new WatchProgressDto
        {
            ...
            VideoTitle = canonicalVideo?.Title ?? "Unknown",
