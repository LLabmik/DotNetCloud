...
/// <summary>
/// Lifecycle gRPC service for the Video module.
/// Allows the core supervisor to initialize, start, stop, health-check, and inspect the module.
/// </summary>
public sealed class VideoLifecycleService : ModuleLifecycle.ModuleLifecycleBase
{
    private readonly VideoModule _module;
    private readonly ILogger<VideoLifecycleService> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="VideoLifecycleService"/> class.
    /// </summary>
    public VideoLifecycleService(VideoModule module, ILogger<VideoLifecycleService> logger)
    {
        _module = module;
        _logger = logger;
    }
    ...
    public override async Task<InitializeResponse> Initialize(InitializeRequest request, ServerCallContext context)
    {
        try
        {
            _logger.LogInformation("Initializing Video module via gRPC: {ModuleId}", request.ModuleId);
        ...
        {
            _logger.LogError(ex, "Failed to initialize Video module");
    ...
    public override async Task<StartResponse> Start(StartRequest request, ServerCallContext context)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to start Video module"); return new StartResponse { Success = false, ErrorMessage = ex.Message }; }
    }
    ...
    public override async Task<StopResponse> Stop(StopRequest request, ServerCallContext context)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to stop Video module"); return new StopResponse { Success = false, ErrorMessage = ex.Message }; }
    }

    /// <inheritdoc />
    public override Task<HealthCheckResponse> HealthCheck(HealthCheckRequest request, ServerCallContext context)
    {
        var response = new HealthCheckResponse { Status = HealthStatus.Healthy, Description = "Video module is healthy" };
