...
/// </summary>
public sealed class ExampleLifecycleService : ModuleLifecycle.ModuleLifecycleBase
{
    private readonly ExampleModule _module;
    private readonly ILogger<ExampleLifecycleService> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="ExampleLifecycleService"/> class.
    /// </summary>
    public ExampleLifecycleService(ExampleModule module, ILogger<ExampleLifecycleService> logger)
    {
        _module = module;
        _logger = logger;
    }
        ...
        InitializeRequest request, ServerCallContext context)
    {
        try
        {
            _logger.LogInformation("Initializing example module via gRPC: {ModuleId}", request.ModuleId);
        ...
        {
            _logger.LogError(ex, "Failed to initialize example module");
        ...
        StartRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to start example module");
        ...
        StopRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to stop example module");
        ...
        HealthCheckRequest request, ServerCallContext context)
    {
        var response = new HealthCheckResponse
        {
            ...
            Description = "Example module is healthy"
        };
    ...
    {
        // In a real module host, the IServiceProvider comes from the hosting DI container.
        // The ServerCallContext provides access to the host's HttpContext.
