...
/// so the core supervisor can control the Calendar module's lifecycle and check health.
/// </summary>
public sealed class CalendarLifecycleService : ModuleLifecycle.ModuleLifecycleBase
{
    private readonly CalendarModule _module;
    private readonly ILogger<CalendarLifecycleService> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="CalendarLifecycleService"/> class.
    /// </summary>
    public CalendarLifecycleService(CalendarModule module, ILogger<CalendarLifecycleService> logger)
    {
        _module = module;
        _logger = logger;
    }
        ...
        InitializeRequest request, ServerCallContext context)
    {
        try
        {
            _logger.LogInformation("Initializing Calendar module via gRPC: {ModuleId}", request.ModuleId);
        ...
        StartRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to start Calendar module");
        ...
        StopRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to stop Calendar module");
        ...
        HealthCheckRequest request, ServerCallContext context)
    {
        var response = new HealthCheckResponse
        {
            ...
            Description = "Calendar module is healthy"
        };
