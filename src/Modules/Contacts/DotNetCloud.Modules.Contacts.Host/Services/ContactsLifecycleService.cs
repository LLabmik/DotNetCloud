...
/// so the core supervisor can control the Contacts module's lifecycle and check health.
/// </summary>
public sealed class ContactsLifecycleService : ModuleLifecycle.ModuleLifecycleBase
{
    private readonly ContactsModule _module;
    private readonly ILogger<ContactsLifecycleService> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="ContactsLifecycleService"/> class.
    /// </summary>
    public ContactsLifecycleService(ContactsModule module, ILogger<ContactsLifecycleService> logger)
    {
        _module = module;
        _logger = logger;
    }
        ...
        InitializeRequest request, ServerCallContext context)
    {
        try
        {
            _logger.LogInformation("Initializing Contacts module via gRPC: {ModuleId}", request.ModuleId);
        ...
        {
            _logger.LogError(ex, "Failed to initialize Contacts module");
        ...
        StartRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to start Contacts module");
        ...
        StopRequest request, ServerCallContext context)
    {
        ...
        {
            _logger.LogError(ex, "Failed to stop Contacts module");
        ...
        HealthCheckRequest request, ServerCallContext context)
    {
        var response = new HealthCheckResponse
        {
            ...
            Description = "Contacts module is healthy"
        };
