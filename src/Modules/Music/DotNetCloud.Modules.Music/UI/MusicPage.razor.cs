...
public partial class MusicPage : IAsyncDisposable
{
    ...
    private async Task RequestRemoveLibrarySourceAsync(MediaLibrarySource source)
    {
        ...
        try
        {
            ...
            var count = await MediaLibraryScanner.CountLibraryItemsNotInSourcesAsync(remainingSources, _caller.UserId, "Music");
            if (count == 0)
            {
        ...
        {
            Logger.LogWarning(ex, "Music source removal preview failed for user {UserId}", _caller.UserId);
        ...
        bool attemptPrune)
    {
        ...
        try
        {
            var removed = await MediaLibraryScanner.RemoveLibraryItemsNotInSourcesAsync(remainingSources, _caller.UserId, "Music");
        ...
        {
            Logger.LogWarning(ex, "Music library prune after source removal failed for user {UserId}", _caller.UserId);
