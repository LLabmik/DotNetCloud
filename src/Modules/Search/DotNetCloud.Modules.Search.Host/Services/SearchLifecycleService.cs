...
/// <summary>
/// Lifecycle gRPC service for the Search module.
/// Allows the core supervisor to initialize, start, stop, health-check, and inspect the module.
/// </summary>
public sealed class SearchLifecycleService : ModuleLifecycle.ModuleLifecycleBase
{
    private readonly SearchModule _module;
    private readonly ILogger<SearchLifecycleService> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="SearchLifecycleService"/> class.
    /// </summary>
    public SearchLifecycleService(SearchModule module, ILogger<SearchLifecycleService> logger)
    {
        _module = module;
        _logger = logger;
    }
    ...
    public override async Task<InitializeResponse> Initialize(InitializeRequest request, ServerCallContext context)
    {
        try
        {
            _logger.LogInformation("Initializing Search module via gRPC: {ModuleId}", request.ModuleId);
        ...
        {
            _logger.LogError(ex, "Failed to initialize Search module");
    ...
    public override async Task<StartResponse> Start(StartRequest request, ServerCallContext context)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to start Search module"); return new StartResponse { Success = false, ErrorMessage = ex.Message }; }
    }
    ...
    public override async Task<StopResponse> Stop(StopRequest request, ServerCallContext context)
    {
        ...
        catch (Exception ex) { _logger.LogError(ex, "Failed to stop Search module"); return new StopResponse { Success = false, ErrorMessage = ex.Message }; }
    }

    /// <inheritdoc />
    public override Task<HealthCheckResponse> HealthCheck(HealthCheckRequest request, ServerCallContext context)
    {
        var response = new HealthCheckResponse { Status = HealthStatus.Healthy, Description = "Search module is healthy" };
