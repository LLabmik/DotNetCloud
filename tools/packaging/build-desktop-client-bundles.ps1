...
$LinuxInstallScript = @(
        ...
        '',
        '# Select the install model: root -> system-wide under /opt; a normal user gets',
        '# a per-user install under ~/.local/share (no root required, direct-copy updates).',
        'if [[ "$EUID" -eq 0 ]]; then',
        '    MODE="system"',
        ...
        '    INSTALL_DIR="$DATA_HOME/dotnetcloud-desktop-client"',
        '    SERVICE_NAME=""',
        '    SERVICE_FILE=""',
        '    LAUNCHER="$BIN_HOME/dotnetcloud-sync-tray"',
        '    APPLICATIONS_DIR="$DATA_HOME/applications"',
        '    ICON_THEME_ROOT="$DATA_HOME/icons/hicolor"',
        'fi',
        '',
        'ICON_THEME_DIR="$ICON_THEME_ROOT/scalable/apps"',
        ...
        '# (System installs only; per-user installs never register a service.)',
        ...
        'fi',
        '',
        '# ~/.local/bin may not exist yet on a fresh per-user install.',
        ...
        '# (System installs only: per-user installs land in directories the desktop',
        '#  environment watches already, so no disruptive session restart is needed.)',
        'if [[ "$MODE" == "system" ]]; then',
        '    if [[ -n "${SUDO_USER:-}" ]]; then',
        ...
        '    fi',
        'fi',
        'if [[ "$MODE" == "system" && -n "${DBUS_ADDR:-}" ]]; then',
        ...
        'echo',
        'if [[ "$MODE" == "user" ]]; then',
        '    echo "  Install type: per-user ($INSTALL_DIR)"',
        '    echo "  Uninstall with: ./uninstall.sh"',
        'else',
        '    echo "  Install type: system-wide ($INSTALL_DIR)"',
        '    echo "  Uninstall with: sudo ./uninstall.sh"',
        'fi',
        'echo',
...
$LinuxUninstallScript = @(
        ...
        '    DESKTOP_FILE="$APPLICATIONS_DIR/dotnetcloud-sync-tray.desktop"',
        '    ICON_THEME_ROOT="/usr/share/icons/hicolor"',
        '    ICON_INSTALLED="$ICON_THEME_ROOT/scalable/apps/dotnetcloud-sync-tray.svg"',
        'else',
        '    MODE="user"',
        '    DATA_HOME="${XDG_DATA_HOME:-$HOME/.local/share}"',
        '    BIN_HOME="${XDG_BIN_HOME:-$HOME/.local/bin}"',
        ...
        'fi',
        '',
        'if [[ -n "$SERVICE_NAME" ]] && command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files 2>/dev/null | grep -q "^${SERVICE_NAME}\.service"; then',
        '  systemctl stop "$SERVICE_NAME" 2>/dev/null || true',
        '  systemctl disable "$SERVICE_NAME" 2>/dev/null || true',
        'fi',
        'if [[ -n "$SERVICE_FILE" ]]; then',
        ...
        'rm -f "$DESKTOP_FILE"',
        'rm -f "$ICON_INSTALLED"',
        '# Remove the app-managed autostart entry for per-user uninstalls.',
        'if [[ "$MODE" == "user" ]]; then',
        '    rm -f "${XDG_CONFIG_HOME:-$HOME/.config}/autostart/dotnetcloud-sync-tray.desktop"',
        'fi',
        'if command -v gtk-update-icon-cache >/dev/null 2>&1 && [[ -f "$ICON_THEME_ROOT/index.theme" ]]; then',
        '  gtk-update-icon-cache -f -t "$ICON_THEME_ROOT" 2>/dev/null || true',
        'fi',
        ...
        '',
        'if [[ "$MODE" == "user" ]]; then',
        '  echo "DotNetCloud Sync Client (per-user) has been uninstalled."',
        'else',
        '  echo "DotNetCloud Sync Client (system-wide) has been uninstalled."',
        'fi'
) -join [Environment]::NewLine
...
Set-Content -Path (Join-Path $LinuxRoot "README.txt") -Value ((@(
        ...
        '',
        'Install (per-user, recommended - no root required):',
        '  ./install.sh',
        '',
        'Install (system-wide for all users, /opt):',
        '  sudo ./install.sh',
        '',
        'Uninstall (per-user):',
        '  ./uninstall.sh',
        ...
        '  sudo ./uninstall.sh',
        '',
        'After install, run tray in your session:',
