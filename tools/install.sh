...
# --- Write Collabora environment variables consumed by the systemd unit ---
# The dotnetcloud.service unit references this file via EnvironmentFile=/etc/dotnetcloud/env.
# It is also written by 'dotnetcloud setup' (SetupCommand.cs) on every setup run.
write_collabora_env_file() {
    local config_file="$1"
    ...
    local public_origin
    public_origin=$(resolve_public_origin_from_config "$config_file" 2>/dev/null || true)
    if [[ -z "$public_origin" ]]; then
        warn "Could not resolve public origin; skipping Collabora env file."
        return
    fi
    ...
    # Read admin email and MFA preference from config.json
    local admin_email
    admin_email=$(grep -oP '"(?:adminEmail|AdminEmail)"\s*:\s*"\K[^"]+' "$config_file" 2>/dev/null | head -n1 || true)
    local admin_mfa_enabled
    admin_mfa_enabled=$(grep -oP '"(?:enableAdminMfa|EnableAdminMfa)"\s*:\s*\K(true|false)' "$config_file" 2>/dev/null | head -n1 || echo "false")

    $SUDO tee "${CONFIG_DIR}/env" > /dev/null <<EOF
# DotNetCloud runtime environment (generated by installer)

# Admin credentials (used by AdminSeeder on first-run and existing-user setup)
DotNetCloud__AdminEmail=${admin_email}
DotNetCloud__AdminMfaEnabled=${admin_mfa_enabled}

# Collabora Online
Files__Collabora__Enabled=true
Files__Collabora__UseBuiltInCollabora=false
Files__Collabora__ServerUrl=${public_origin}
...
EOF
    ...
    ok "Collabora env file written to ${CONFIG_DIR}/env."
}
...
maybe_install_collabora() {
    ...
    if [[ "$MODE" == "BuiltIn" ]]; then
        ...
        # Write Collabora environment file for the systemd unit's EnvironmentFile reference.
        write_collabora_env_file "$CONFIG_FILE"
